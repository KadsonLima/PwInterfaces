import { imagesDefault, imagesFlamengo, imagesMagenta, imagesMagentaBlue, imagesNaruto } from "../assets/produtos/naruto";

// {
//   id: 0,
//   name: "Naruto",
//   description: "Interface Naruto",
//   youtube: "jXGRL3J87B4",
//   images: [""],
//   link:"https://mpago.la/2yt3oNX",
//   valor: "50,00R$"
// },

export const products = [
  {
    id: 0,
    name: "Interface Padrão - Mapa Grande, Daimon Reta e Esconde PT",
    description: "Interface Padrão com Mapa Grande, daimon reta e o T modificado para esconder PT, sendo assim compatibilidade de 1.5.1-1.5.9, além de auto update reduzindo a possibilidade de bugs.",
    youtube: "KO3kwNtTsCw",
    images: imagesDefault,
    link:"https://mpago.la/124JewL",
    valor: "25,00R$"
  },
  {
    id: 1,
    name: "Magenta Ui - Para Telas Pequenas Archy's",
    description: "Magenta Ui - Para Telas Pequenas, Com o novo sistema de Instalação, sendo assim compatibilidade de 1.5.1-1.5.9, além de auto update reduzindo a possibilidade de bugs, interface feita por Archy's",
    youtube: "e2_M43-RP9g",
    images: imagesMagenta,
    link:"https://mpago.la/2yt3oNX",
    valor: "50,00R$"
  },
  {
    id: 2,
    name: "Magenta Ui Blue- Para Telas Pequenas Archy's",
    description: "Magenta Ui Blue- Para Telas Pequenas, Com o novo sistema de Instalação, sendo assim compatibilidade de 1.5.1-1.5.9, além de auto update reduzindo a possibilidade de bugs, interface feita por Archy's",
    youtube: "e2_M43-RP9g",
    images: imagesMagentaBlue,
    link:"https://mpago.la/28mzkkd",
    valor: "50,00R$"
  },
  {
    id: 3,
    name: "Flamengo Ui",
    description: "Flamengo, Com o novo sistema de Instalação, sendo assim compatibilidade de 1.5.1-1.5.9, além de auto update reduzindo a possibilidade de bugs, interface feita por Archy's",
    youtube: "2DUZmind1pQ",
    images: imagesFlamengo,
    link:"https://mpago.la/1qkkdqx",
    valor: "50,00R$"
  },
];
